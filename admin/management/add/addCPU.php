<?php
    ...
    if($uploadOk == 0){
        $query ="INSERT INTO `products`(`productID`,`name`, `price`, `quantity`, `model`, `manufacturer`, `categoryID`) VALUES (?,?,?,?,?,?, 1);";
        ...
        $query ="INSERT INTO `products`(`productID`,`name`, `price`, `quantity`, `model`, `manufacturer`, `categoryID`, `img_src`) VALUES (?,?,?,?,?,?,1,?);";
        $connection->prepare($query)->execute(array($id, $name, $price, $quantity, $model, $manufacturer, $srcAddress));
    }

    $query2 ="INSERT INTO `cpus` (`productID`, `base_clock`, `boost_clock`, `core_count`, `thread_count`, `series`, `socketID`) VALUES (?,?,?,?,?,?,?);";
    
    $connection->prepare($query2)->execute(array($id, $base_clock, $boost_clock, $core_count, $thread_count, $series, $socket));
